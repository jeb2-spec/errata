...
INSERT INTO meta VALUES ('built_on','2026-09-06');
INSERT INTO meta VALUES ('source_commit','11ce5f1a');
...
INSERT INTO meta VALUES ('counts','14 principles, 47 corrections, 63 lessons, 120 article revisions across 38 articles');
INSERT INTO meta VALUES ('integrity_sha256','d4d4a83c83e84ceeab6a9a1957894a4825f3d987e038519d2ad92ecc2d6f7cb8');
...
INSERT INTO corrections VALUES ('a-clock-that-only-ran-on-one-row','2026-09-05','The anchors table''s row for the 2026-09-04 build, and this project''s own check for a stamp nobody is going to make','That the row reading owed and not stamped yet. The current build. described the record''s newest build, and that the tool reporting the record healthy had looked at it.','It had not been the current build for a day. Two later builds superseded it and neither supersession touched the row, so it went on introducing itself as the current build in the published table while the file it named sat unstamped. The tool said OK the whole time, and the reason is the more useful half of this row. The abandonment check exists exactly for a stamp that has stopped being a pending action, and it was written inside a branch that runs only when the owed row is also the current build. The current build is resolved by looking first for the row that points at the live proof file and only then for a row saying owed, so the moment a newer build was stamped it took the current slot and the older owed row lost its clock entirely. Nothing was counting its days. A gate that only inspects the case it was written for is not inspecting the case it exists for, and the case it exists for is the one that arrives when attention has moved on. Note which half held: the sentence under the table is derived and it counted the unstamped build correctly, saying one is not stamped yet. What was wrong was the hand written identification inside the row, which is the same split this record recorded a day earlier as a hand written statement of current state is a number typed in words. The debt is paid here rather than written off. The exact file was recovered from commit 89f26b95, which carries it byte for byte, and stamped against those bytes, and the row now says plainly that the proof attests the file existed by the day it was stamped and not by the day it was built, because for the earlier date the evidence is only the commit history and a reader should not be left to infer more than the proof carries.','in our favour','the reader','Noticed while flipping six confirmed proofs into the same table, which is adjacent work rather than a check aimed at this row. Nobody pointed at it and nothing failed. It was then confirmed by reading the resolution order in the tool''s own source and by reverting the row to see whether the tool would say anything, which it did not.',1);
...
INSERT INTO detections VALUES ('a-clock-that-only-ran-on-one-row','self-audit','post-publication',1,'Found while flipping six other rows in the same table, so the attention was in the neighbourhood but not on this row, and nothing had failed to prompt it. Contemporaneous. Scored independent, with the qualifier that adjacent work is a weaker kind of unprompted than a pass aimed at nothing in particular, and this table has no column for that distinction.');
